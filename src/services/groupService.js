...
import { collection, addDoc, getDocs, doc, getDoc, updateDoc, deleteDoc, arrayUnion, query, where } from "firebase/firestore";
...
export default {
 ...
 async deleteGroup(groupId) {
  ...
  async deleteGroupWithExpenses(groupId) {
    ...
    const q = query(expensesRef, where("groupId", "==", groupId));
    const snapshot = await getDocs(q);
    const batchDeletes = snapshot.docs.map(expDoc => deleteDoc(doc(db, "expenses", expDoc.id)));
